...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT s.student_id, s.full_name, s.email, s.contact_number, s.program, s.photo, 
                        s.father_name, s.mother_name, s.current_address, s.permanent_address, f.total_fee, f.paid_fee, f.due_fee
                FROM students s
                LEFT JOIN fee_details f ON s.student_id = f.student_id
                WHERE s.student_id = ?";
